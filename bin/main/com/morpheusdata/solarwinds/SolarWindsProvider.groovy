...
class SolarWindsProvider implements IPAMProvider {
        ...
        try {
            ...
            if(hostOnline) {
                ...
                if(subnetList.success) {
                    ...
                    rtn.msg = subnetList.msg ?: 'Error connecting to SolarWinds'
                }
            } else {
    ...
    void cacheNetworks(HttpApiClient client, NetworkPoolServer poolServer, Map opts = [:]) {
        ...
        if(listResults.success && listResults.data != null) {
            List apiItems = listResults.data?.results?.findAll{['Subnet','IPv6 Subnet'].contains(it.GroupTypeText)}?.unique{it.SubnetId} as List<Map>
    ...
    void addMissingPools(NetworkPoolServer poolServer, Collection<Map> chunkedAddList) {
        ...
        def poolTypeIpv6 = new NetworkPoolType(code: 'solarwindsipv6')
        ...
        chunkedAddList?.each { Map it ->
            ...
            def newNetworkPool
            if(networkIp) {
                if(it.GroupTypeText == 'Subnet') {
                    networkInfo = getNetworkPoolConfig(networkIp)
                ...
                if (it.GroupTypeText == 'IPv6 Subnet') {
                    def addConfig = [poolServer: poolServer,account:poolServer.account, owner:poolServer.account, name:displayName, externalId:it.SubnetId,
                                    displayName:displayName, type:poolTypeIpv6, cidr: networkIp, poolEnabled:true, parentType:'NetworkPoolServer', parentId:poolServer.id]
                    newNetworkPool = new NetworkPool(addConfig)
                    newNetworkPool.ipRanges = []
                    rangeConfig = [cidrIPv6: networkIp, startIPv6Address: it.Address, endIPv6Address: it.Address]
                    addRange = new NetworkPoolRange(rangeConfig)
                    newNetworkPool.ipRanges.add(addRange)
                }
    ...
    Collection<NetworkPoolType> getNetworkPoolTypes() {
        return [
                ...
                new NetworkPoolType(code:'solarwindsipv6', name:'SolarWinds IPv6', creatable:false, description:'SolarWinds IPv6', rangeSupportsCidr: true, ipv6Pool:true)
        ]
    }
    ...
    List<OptionType> getIntegrationOptionTypes() {
        return [
                ...
                new OptionType(code: 'networkPoolServer.solarwindsipam.serviceUsername', name: 'Service Username', inputType: OptionType.InputType.TEXT, fieldName: 'serviceUsername', fieldLabel: 'Username', fieldContext: 'domain', displayOrder: 2, localCredential: true, required: true),
                new OptionType(code: 'networkPoolServer.solarwindsipam.servicePassword', name: 'Service Password', inputType: OptionType.InputType.PASSWORD, fieldName: 'servicePassword', fieldLabel: 'Password', fieldContext: 'domain', displayOrder: 3, localCredential: true, required: true),
...
//        ]
        ...
        requestOptions.queryParams = [query: 'SELECT SubnetId,ParentId,FriendlyName,Address,CIDR,GroupTypeText FROM IPAM.Subnet']
